...
private boolean usuarioDEV;
	...
	public boolean isUsuarioDEV() {return usuarioDEV;}
	public void setUsuarioDEV(boolean usuarioDEV) {	this.usuarioDEV = usuarioDEV;}
	
	
	public boolean pertenceAoGrupo(Connection connector, String codGroup){
	
	if(this.isUsuarioDEV())
	return true;
	...
	return false;
		...
		"SELECT uxg.id FROM "+Comuns.NOME_TB_USUARIOS_X_GRUPO+" as uxg inner join "+Comuns.NOME_TB_GRUPOS+" as g on g.id=uxg.fk_grupo WHERE uxg.fk_usuario= ? and g.cod = ?");
	...
	boolean control = false;	
		...
		PreparedStatement statement = (PreparedStatement) connector.prepareStatement("SELECT g.cod FROM "+Comuns.NOME_TB_GRUPOS+" as g inner join "+Comuns.NOME_TB_USUARIOS_X_GRUPO+" as uxg on g.id=uxg.fk_grupo WHERE uxg.fk_usuario= ?");
		statement.setInt(1,  this.getId());
		ResultSet set = statement.executeQuery();	
			...
			statement = (PreparedStatement) connector.prepareStatement("SELECT gxa.valor, a.tipo FROM "+Comuns.NOME_TB_GRUPO_X_ACESSO+" as gxa inner join "+Comuns.NOME_TB_ACESSOS+" as a on a.id=gxa.fk_acesso WHERE gxa.fk_grupo IN (select fk_grupo from "+Comuns.NOME_TB_USUARIOS_X_GRUPO+" where fk_usuario=?) and a.cod = ?");
			statement.setInt(1,  this.getId());
