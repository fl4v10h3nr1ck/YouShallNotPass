...
public class LoginDAO extends DAO{
	...
	public boolean validaBD(){
		...
		DatabaseMetaData metaData = this.conector.getMetaData();  
		ResultSet tables = metaData.getTables(null, null, null, new String[] { "TABLE_NAME" });  
		...
		boolean tb_uxg = false;
			...
			if( tables.getString(3).compareTo(Comuns.NOME_TB_USUARIOS) == 0)
			tb_users = true;
			
			if( tables.getString(3).compareTo(Comuns.NOME_TB_ACESSOS) == 0)
			tb_accesses = true;
			
			if( tables.getString(3).compareTo(Comuns.NOME_TB_GRUPOS) == 0)
			tb_groups = true;
			
			if( tables.getString(3).compareTo(Comuns.NOME_TB_GRUP_ACESS) == 0)
			tb_gxa = true;
			
			if( tables.getString(3).compareTo(Comuns.NOME_TB_USER_GRUP) == 0)
			tb_uxg = true;
			}
	...
	public boolean criaTabelasDeLogin(ListaDeRecursosDoSistema listaDeRecursosDoSistema){
		...
		stmt.executeUpdate("CREATE TABLE IF NOT EXISTS "+Comuns.NOME_TB_USER_GRUP+" (id INT NOT NULL AUTO_INCREMENT, fk_usuario INT NOT NULL, fk_grupo INT NOT NULL, PRIMARY KEY (id), INDEX fk_usuario_idx (fk_usuario ASC), INDEX fk_grupo_idx (fk_grupo ASC)) ENGINE = MyISAM;");
		stmt.executeUpdate("CREATE TABLE IF NOT EXISTS "+Comuns.NOME_TB_GRUP_ACESS+" (id INT NOT NULL AUTO_INCREMENT, fk_grupo INT NOT NULL, fk_acesso INT NOT NULL, PRIMARY KEY (id), valor VARCHAR(45), INDEX fk_acesso_idx (fk_acesso ASC), INDEX fk_grupo_idx (fk_grupo ASC)) ENGINE = MyISAM;");
		...
		statement = (PreparedStatement) this.conector.prepareStatement( 			      		
		"INSERT INTO "+Comuns.NOME_TB_USER_GRUP+"("
		+ "fk_usuario, fk_grupo)"+
		" VALUES ("
		+ "?,?)");   
		...
		catch (SQLException e) {return this.mensagemDeErroDeAcessoAoBD();}	
